...
public class csFriendsDbRepos
{
    ...
    #endregion
    public async Task<List<csFriendDbM>> AllInfoAsync()
    {
        var db = csMainDbContext.DbContext(_dblogin);
        var info = db.Friends.Select(f => f).ToList();
        return info;
    }
    ...
    */
    ...
    {
        if (itemDto.FriendId != Guid.Empty)
    ...
    {
        if (itemDto.AddressId != Guid.Empty)
    ...
    public async Task<IQuote> CreateQuoteAsync(loginUserSessionDto usr, csQuoteCUdto itemDto)
    {
        ...
        using (var db = csMainDbContext.DbContext(_dblogin))
        {
            ...
            /*await csQuoteCUdto_to_csQuoteDbM(db, itemDto, _item);*/
...
/*    private static async Task csQuoteCUdto_to_csQuoteDbM(csMainDbContext db, csQuoteCUdto _itemDtoSrc, csQuoteDbM _itemDst)
    {
        if (_itemDtoSrc.FriendId != null)
        {
            var result = db.Friends.First(i => i.FriendId == _itemDtoSrc.FriendId);
           var _friend = new List<csFriendDbM>();
            _friend.Add(result);
            _itemDst.FriendsDbM = _friend;
        } 
    }*/
    #endregion
